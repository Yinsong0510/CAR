...
def run(datapath, imgshape, model_dir=""):
    torch.cuda.set_device("cuda:0")
    device = torch.device("cuda")

    transform_nearest = SpatialTransformer(imgshape, mode="nearest").to(device)
    name_1 = sorted(glob.glob(os.path.join(datapath, "T1w", "sub-CC*_T1w_unbiased.nii.gz")))
    name_2 = sorted(glob.glob(os.path.join(datapath, "T2w", "sub-CC*_T2w_unbiased.nii.gz")))
    labels = sorted(glob.glob(os.path.join(datapath, "seg", "sub-CC*_T1w_unbiased_seg.nii.gz")))
    dataloader = Data.DataLoader(Dataset_epoch_validation(name_1, name_2, labels, norm=True), batch_size=1, shuffle=False, num_workers=1)

    log_file = os.path.join(model_dir, "Results.txt")
    with open(log_file, "a") as log:
        log.write("Validation Results log:\n")

    models = sorted(glob.glob(os.path.join(model_dir, "CONTRA_AUG_NCC_*.pth")))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Validation script for CAR models")
