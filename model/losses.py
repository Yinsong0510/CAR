...
class NCC_Var:
    ...
    def loss(self, y_true, y_pred, var):
        ...
        cc = cross * cross / (I_var * J_var + 1e-5) * var
...
class MSE_var:
    ...
    def loss(self, y_true, y_pred, var):
        diff = torch.mean(((y_true - y_pred) ** 2) * var)
        return diff
...
class MSE:
    ...
    def loss(self, y_true, y_pred):
        diff = torch.mean(((y_true - y_pred) ** 2))
        return diff
...
class info_nce_dense:
    ...
    def loss(self, features):
        nb, nc, _, _ = features.shape
        batch_size = nb // 2
        labels = torch.cat([torch.arange(batch_size) for i in range(self.n_views)], dim=0)
        labels = (labels.unsqueeze(0) == labels.unsqueeze(1)).float()
        labels = labels.cuda()
        # positive sample labels 1, negative sample labels 0
        mask = torch.eye(labels.shape[0], dtype=torch.bool).cuda()  # identity matrix with bool value with the same
        # size with similarity matrix
        labels = labels[~mask].view(labels.shape[0], -1)  # label matrix with main diagonal removed where positive
        # sample labels 1, negative sample labels 0

        features = torch.flatten(features, start_dim=2, end_dim=3)
        features = features.permute(2, 0, 1)
        features = F.normalize(features, dim=2)
        transpose_features = torch.transpose(features, 1, 2)

        points = features.shape[0]
        mask = mask.repeat(points, 1, 1)
        labels = labels.repeat(points, 1, 1)

        similarity_matrix = torch.matmul(features, transpose_features)
        # assert similarity_matrix.shape == (
        #     self.args.n_views * self.args.batch_size, self.args.n_views * self.args.batch_size)
        # assert similarity_matrix.shape == labels.shape

        # discard the main diagonal from both: labels and similarities matrix
        similarity_matrix = similarity_matrix[~mask].view(points, similarity_matrix.shape[1], -1)
        # assert similarity_matrix.shape == labels.shape

        # select and combine multiple positives
        positives = similarity_matrix[labels.bool()].view(points, labels.shape[1],
                                                          -1)  # select from similarity matrix where labels is one
        positives = positives.view(-1, positives.shape[2])

        # select only the negatives
        negatives = similarity_matrix[~labels.bool()].view(points, similarity_matrix.shape[1], -1)
        negatives = negatives.view(-1, negatives.shape[2])
...
class DiceLoss(torch.nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        union = torch.pow(y_pred, 1).sum(dim=[2, 3]) + torch.pow(y_true, 1).sum(dim=[2, 3])
...
class MINDLoss(torch.nn.Module):
    ...
    def forward(self, input, target):
        ...
        return l1 / (input.shape[2] * input.shape[3] * self.nl_size)
...
class MIND_2D_loss(torch.nn.Module):
    ...
    def MINDSSC(self, img, radius=2, dilation=2):
        ...
        mshift1 = torch.zeros(4, 1, 3, 3).cuda()
        mshift1.view(-1)[torch.arange(4) * 9 + idx_shift1[:, 0] * 3 + idx_shift1[:, 1]] = 1
        mshift2 = torch.zeros(4, 1, 3, 3).cuda()
        ...
        mind_var = torch.clamp(mind_var, (mind_var.mean() * 0.01).item(), (mind_var.mean() * 100).item())
        mind /= mind_var
        mind = torch.exp(-mind)
...
class correlation_loss(torch.nn.Module):
    ...
    def forward(self, fea, fea_aug):
        fea_flat = fea.view(fea.shape[1], -1)
        fea_aug_flat = fea_aug.view(fea_aug.shape[1], -1)

        # Calculate mean for each channel
        mean_fea = torch.mean(fea_flat, dim=1, keepdim=True)
        mean_aug = torch.mean(fea_aug_flat, dim=1, keepdim=True)

        # Compute the deviation from the mean
        dev_fea = fea_flat - mean_fea
        dev_aug = fea_aug_flat - mean_aug

        # Compute the covariance for each channel
        covariance = torch.mean(dev_fea * dev_aug, dim=1)

        # Compute the standard deviation for each channel
        std_fea = torch.std(fea_flat, dim=1)
        std_aug = torch.std(fea_aug_flat, dim=1)

        # Compute the correlation for each channel
        correlation = covariance / (std_fea * std_aug)
...
class Correlation(nn.Module):
    ...
    def forward(self, x_1, x_2):
        ...
        x_out = torch.cat([torch.mean(x_1 * x_2[:, :, dx:dx + w, dy:dy + h], 1, keepdim=True)
                           for dx, dy, dz in zip(offsetx.reshape(-1), offsety.reshape(-1))], 1)
        return x_out


def cyclic_loss(flow, grid):
    shape = flow.shape[2:]
    new_locs = grid + flow
