...
from utils.Functions import Dataset_Mapping


def parse_args():
    parser = ArgumentParser(description="Contrast-augmented mapping training")
...
def build_dataset(datapath):
    names = sorted(glob.glob(os.path.join(datapath, "P*", "S*")))
    index_pairs = []
    for name in names:
        fixed = sorted(glob.glob(os.path.join(name, "s*_t0.nii.gz")))
        moving = sorted(glob.glob(os.path.join(name, "s*_t0_warped.nii.gz")))
        index_pairs.extend(itertools.product(moving, fixed))
    return Dataset_Mapping(index_pairs, norm=True)


def train(args):
    torch.cuda.set_device("cuda:0")
    enc_nf, dec_nf = [128] * 4, [256] * 7
    model = VxmDense(
    ...
    dataset = build_dataset(args.datapath)
    ...
    writer = SummaryWriter("../Log/CAR_proj_32_mapping")
    model_dir = "checkpoints/CAR_proj_32_mapping"
